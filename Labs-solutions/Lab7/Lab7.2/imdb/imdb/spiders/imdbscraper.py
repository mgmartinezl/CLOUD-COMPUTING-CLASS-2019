    ...
    def parse(self, response):
        ...
        for actor in actors:
            ...
            if len(actor.split("alt=")) > 1:
                ...
                yield Request(url, callback=self.parse_actor)

    def parse_actor(self, response):
        print("holi")
        yield {'birth_date':
                   response.xpath('//script[@type = "application/ld+json"]').extract_first().split('"birthDate":')[
                       1].split('"')[1]}

        movies = response.xpath('//div[@class = "filmo-category-section"]/div').extract()[3]

        for movie in movies:
            yield {'movie_name': movie.split('"year_column">')[1].split("\n")[1], }
